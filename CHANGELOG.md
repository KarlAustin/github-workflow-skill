...
### Fixed
- Pre-flight checks now use `.claude-plugin/plugin.json` to detect installed plugins instead of `skills/` directories (which were removed in v1.2.0)

## [1.3.0] - 2026-02-17

### Fixed
- Moved `reference.md` and `templates/` out of `commands/` into `assets/` — Claude Code was registering `reference.md` as a ghost `wflow:wflow:reference` command
- Only actual command `.md` files remain in `commands/wflow/`
